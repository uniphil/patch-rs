...
pub(crate) fn parse_single_patch(s: &str) -> Result<Patch, ParseError<'_>> {
  ...
  if remaining_input.fragment().is_empty() == false {
    Err(ParseError {
...
fn chunk(input: Input<'_>) -> IResult<Input<'_>, Hunk> {
  ...
  Ok((input, Hunk { old_range, new_range, range_hint, lines }))
}
...
fn chunk_line(input: Input<'_>) -> IResult<Input<'_>, Line> {
  alt((
    map(preceded(tuple((char('+'), not(tag("++ ")))), consume_content_line), Line::Add),
    map(preceded(tuple((char('-'), not(tag("-- ")))), consume_content_line), Line::Remove),
...
fn no_newline_indicator(input: Input<'_>) -> IResult<Input<'_>, bool> {
  map(opt(terminated(tag("\\ No newline at end of file"), opt(line_ending))), |matched| matched.is_some())(input)
}
...
fn file_metadata(input: Input<'_>) -> IResult<Input<'_>, Cow<str>> {
  alt((quoted, map(not_line_ending, |data: Input<'_>| Cow::Borrowed(*data.fragment()))))(input)
}
